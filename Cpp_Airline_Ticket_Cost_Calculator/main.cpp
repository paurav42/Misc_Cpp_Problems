...
FlightTicketSystem*  FlightTicketSystem::Instance(const Airlines air){
    ...
    //Type type = Type.GetType(air.ToString());
    //FlightTicketSystem* temp = Activator.CreateInstance(st);
...
int main(){
    ...
    std::vector<std::string> input_tickets {"United 150.0 Premium", "United 120.0 Economy",
     ...
     for(auto& it:input_tickets){
        ...
        float dist = std::stof(vec[1]);
        FlightTicket tkt(dist, vec[2], vec[0]);
        FlightTicketSystem* ticksys;
        //ticksys->Instance(vec[0]);
        auto inst = ticksys->Instance(tkt.airlines);
        float price =  inst->getTicketPrice(tkt);  //ticksys->getTicketPrice(tkt);

        std::cout<<  price << " for " << vec[0] <<" " << vec[2] <<" " << tkt.distance<<std::endl;
